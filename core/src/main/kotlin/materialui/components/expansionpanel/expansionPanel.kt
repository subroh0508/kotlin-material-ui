...
fun RBuilder.expansionPanel(vararg classMap: Pair<PaperStyle, String>, block: ExpansionPanelElementBuilder<DIV>.() -> Unit)
    = child(ExpansionPanelElementBuilder(expansionPanelComponent, classMap.toList()) { DIV(mapOf(), it) }.apply(block).create())

fun <T: Tag> RBuilder.expansionPanel(vararg classMap: Pair<PaperStyle, String>, factory: (TagConsumer<Unit>) -> T, block: ExpansionPanelElementBuilder<T>.() -> Unit)
