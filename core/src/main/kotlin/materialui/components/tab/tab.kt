...
fun RBuilder.tab(vararg classMap: Pair<ButtonBaseStyle, String>, block: TabElementBuilder<BUTTON>.() -> Unit)
    = child(TabElementBuilder(tabComponent, classMap.toList()) { BUTTON(mapOf(), it) }.apply(block).create())

fun <T: Tag> RBuilder.tab(vararg classMap: Pair<ButtonBaseStyle, String>, factory: (TagConsumer<Unit>) -> T, block: TabElementBuilder<T>.() -> Unit)
